...
namespace dragonBones
{
	...
	void CCDragonBones::reVisit() {
		this->updateArmature( m_Armature );
	}

    void CCDragonBones::updateArmature(Armature*arm)
    {
		//
		std::vector<Slot*> vtpSlot= arm->_slotList;
		for (std::vector<Slot*>::iterator itSlot = vtpSlot.begin(); itSlot != vtpSlot.end(); itSlot++)
		{
			std::vector<Object*> vtpObj = (*itSlot)->getDisplayList();
			for (std::vector<Object*>::iterator it = vtpObj.begin(); it != vtpObj.end(); it++)
			{
				if ( Armature* myObject = dynamic_cast<Armature*>(*it) )
				{
					this->updateArmature( myObject );
				}
				else if ( CocosNode* myObject = dynamic_cast<CocosNode*>(*it) )
				{
					Cocos2dxAtlasNode* myAtlasNode = dynamic_cast<Cocos2dxAtlasNode*>(myObject->getNode());
					myAtlasNode->reVisit();
				}
			}
		}
